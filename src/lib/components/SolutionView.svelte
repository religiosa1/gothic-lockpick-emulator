<script lang="ts">
	...
	$effect(() => {
		...
		solutionHistoryEl?.querySelector(`.moves-history__item[data-idx="${idx}"]`)?.scrollIntoView({
			behavior: "smooth",
			block: "nearest",
		});
	});
...
<div class="solution-view">
	...
	>
		{#if solutionManager.editorState !== EditorStateEnum.lockCreation}
			<li
				class="moves-history__item initial"
	...
	{#if isMobile.current && solutionManager.movesHistory.length && hasRedoItems}
		<button
...
<style>
	...
	.moves-history__item.initial::before {
		content: "";
	}
	.moves-history__item:not(.repeated) {
		counter-reset: repeated-steps;
		counter-increment: repeated-steps;
	}
	.moves-history__item.repeated .history-state-btn {
		margin-left: 0.5em;
	}
